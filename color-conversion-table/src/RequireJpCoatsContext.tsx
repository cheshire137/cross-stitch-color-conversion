...
const RequireJpCoatsContext = createContext<RequireJpCoats | undefined>(
  undefined
)
...
export const RequireJpCoatsProvider = ({children}: PropsWithChildren) => {
  ...
  const contextProps = useMemo(
    () => ({requireJpCoats, setRequireJpCoats}),
    [requireJpCoats, setRequireJpCoats]
  )
  return (
    <RequireJpCoatsContext.Provider value={contextProps}>
      {children}
    </RequireJpCoatsContext.Provider>
  )
}
...
export const useRequireJpCoats = () => {
  ...
  if (context === undefined)
    throw new Error(
      'useRequireJpCoats must be used within a RequireJpCoatsProvider'
    )
  return context
}
